...
def main():
  ...
  L2=np.append(-lam, 0) 
  L3=-L1-L2
  diffop = - (np.diag(L3[:-1] if n == 3 or n == 6 else L3)) - np.diag(L2[:n-1],1) - np.diag(L1[1:n],-1)
  ...
  hfin = Tfin + Lv*q/cp # in temperature units
  print(Lv*q/cp)
  print(Tfin)
  print(hfin)
  print(Tfin.shape)
  print(q.shape)
  F = -D * (1-x**2) * np.gradient(hfin, axis=0) / np.gradient(x, axis=0)
  Fl = -D * (1-x**2) * np.gradient(Lv*qfin/cp, axis=0) / np.gradient(x, axis=0)
  ...
  Fl = Fl_hc + Fl_eddy
  EminusP = np.gradient(Fl, axis=0) / np.gradient(x, axis=0)
