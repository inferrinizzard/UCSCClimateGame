...
def model(D=0.5,F=0):
  ...
  Efin = E100[:,-100:]
  Tfin = T100[:,-100:]
  print('Global mean annual mean surface temperature {:.2f} '.format(np.mean(Tfin, axis=(0,1))))
...
def hydro_cycle(Tfin, D, x):
  ...
  Fa = -D * (1-x**2) * np.gradient(hfin, axis=0, edge_order=2) / np.gradient(x, axis=0, edge_order=2)
  Fl = -D * (1-x**2) * np.gradient(Lv*qfin/cp, axis=0, edge_order=2) / np.gradient(x, axis=0, edge_order=2)
  ...
  Fltot = Fl_hc + Fl_eddy
  EminusP = np.gradient(Fltot, axis=0, edge_order=2) / np.gradient(x, axis=0, edge_order=2)
